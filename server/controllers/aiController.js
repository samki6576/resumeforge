...
exports.generateBulletPoints = async (req, res) => {
    try {
        ...
        if (!process.env.GROQ_API_KEY) {
            return res.status(400).json({
                success: false,
                message: 'GROQ_API_KEY is not configured. Please add your API key to .env file.'
            });
        }

        console.log('🤖 Generating bullet points for:', jobTitle, 'at', company);

        const prompt = 'Generate 4-5 professional bullet points for a ' + jobTitle + ' at ' + company + '. Description: ' + description + '. Return ONLY a JSON array of strings. Example: ["Led team of 5 developers...", "Improved performance by 40%..."]';
